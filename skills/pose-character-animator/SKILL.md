...
## Fast path: single pose from JSON + reference

When the user provides a `pose-payload.json` and a reference image and asks for one final frame:
- Skip beat planning.
- Build one direct render prompt from pose coordinates + character description.
- Enforce transparent background if user asks for it (including typo variants).
- Return one command for `skills/imagegen/scripts/image_gen.py edit`.

Use `pose-ui/scripts/render_pose.py` to automate this path.
...
For fast single-pose mode, return:
1. Final prompt
2. Render command
3. One-line checklist
